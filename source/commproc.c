...
if(glProcInfo.stTranLog.ucTranType==PLNPRA){
		memcpy(glProcInfo.stTranLog.szAccount, &glProcInfo.stTranLog.szAccount[0], 12);
		memcpy(glProcInfo.stTranLog.szNilai, &glRecvPack.szBit39[2+12], 12);
		memcpy(glProcInfo.stTranLog.szAdmin, &glRecvPack.szBit39[2+24], 12);
		memcpy(glProcInfo.stTranLog.szToken, &glRecvPack.szBit39[2+36], 20);
		memcpy(glProcInfo.stTranLog.szNamaPelanggan, &glRecvPack.szBit39[2+56], 25);
		PubTrimHeadChars(glProcInfo.stTranLog.szNamaPelanggan,'0');
	}else if(glProcInfo.stTranLog.ucTranType==INQPINTERPAY||glProcInfo.stTranLog.ucTranType==PINTERPAY||glProcInfo.stTranLog.ppaj>0){
		ushort n;
		memcpy(glProcInfo.stTranLog.szMerchName, &glRecvPack.szBit39[2], 50);
		memcpy(glProcInfo.stTranLog.szMemberName, &glRecvPack.szBit39[2+50], 50);
		memcpy(glProcInfo.stTranLog.szPackageName, &glRecvPack.szBit39[2+100], 50);
		memcpy(glProcInfo.stTranLog.szTrxReff, &glRecvPack.szBit39[2+150], 8);
		memcpy(glProcInfo.stTranLog.szPackagePrice, &glRecvPack.szBit39[2+158], 20);
		memcpy(glProcInfo.stTranLog.szPackageDiscount, &glRecvPack.szBit39[2+178], 20);
		memcpy(glProcInfo.stTranLog.szPackagePoin, &glRecvPack.szBit39[2+198], 20);
		PubTrimHeadChars(glProcInfo.stTranLog.szMerchName,' ');
		PubTrimHeadChars(glProcInfo.stTranLog.szMemberName,' ');
		PubTrimHeadChars(glProcInfo.stTranLog.szPackageName,' ');
		PubTrimHeadChars(glProcInfo.stTranLog.szPackagePrice,' ');
		PubTrimHeadChars(glProcInfo.stTranLog.szPackageDiscount,' ');
		PubTrimHeadChars(glProcInfo.stTranLog.szPackagePoin,' ');
		/*for(n=0;n<counterPackage;n++){
			if(memcmp(glProcInfo.stTranLog.szTrxReff,glPackageList[n].sPackageCode,8)==0){
				glPackageList[n].price=atoi(glRecvPack.szTranAmt);
				break;
			}
		}*/
	}else if(glProcInfo.stTranLog.ucTranType==INQPPOB||glProcInfo.stTranLog.ucTranType==INQTRANS||glProcInfo.stTranLog.ucTranType==PPOB||glProcInfo.stTranLog.ucTranType==PULSA){
		uchar szBuff[3];
		uchar szBuffer[22];
		...
		for(n=0;n<responseCounter;n++){
			...
			while(1){
				if(glProcInfo.stTranLog.ucTranType==INQPPOB||glProcInfo.stTranLog.ucTranType==INQTRANS)headl=4;
				if(glProcInfo.stTranLog.ucTranType==PPOB||glProcInfo.stTranLog.ucTranType==PULSA)headl=7;
				if(memcmp(&glRecvPack.szBit62[headl+i],";",1)==0){
					i++;
					a=0;
					break;
				}
				memcpy(&glProcInfo.stTranLog.szParamName[a],&glRecvPack.szBit62[headl+i],1);
				a++;
				i++;
			}
			while(1){
					if(memcmp(&glRecvPack.szBit63[headl+ii],";",1)==0){
						ii++;
						b=0;
						break;
					}
					memcpy(&glProcInfo.stTranLog.szParamValue[b],&glRecvPack.szBit63[headl+ii],1);
					b++;
					ii++;
			}
			if(glProcInfo.stTranLog.ucTranType==INQPPOB&&memcmp(glProcInfo.stTranLog.szParamName,"REF NUM",7)==0){
				sprintf((char *)glProcInfo.stTranLog.szTrxReff,"%s",glProcInfo.stTranLog.szParamValue);
				break;
			}
			/*if(glProcInfo.stTranLog.ucTranType==INQPPOB&&memcmp(glProcInfo.stTranLog.szParamName,"REFF",4)==0){
				sprintf(glProcInfo.stTranLog.szTrxReff,'%s',glProcInfo.stTranLog.szParamValue);
				break;
			}*/
		}	
		}
